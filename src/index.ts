...
type BlurFn<T> = (event: React.FocusEvent<HTMLInputElement>, props: BlurEvent<T>) => void;
...
const useForm = <T extends { [key in keyof T]: InputTypes }>(
	...
	const values = useMemo(() => Object.values(state.fields), [state.fields]);

	const setState = useCallback((newProps: Partial<T>) => dispatch.setState(newProps), []);
